...
const ProductImageWithOverlayComponent: React.FC<Props> = ({ imageSrc, index = 0, carouselImages: imagesFromApi }) => {
    const [mounted, setMounted] = React.useState(false);
    const [productCarouselApi, setProductCarouselApi] = React.useState<CarouselApi | null>(null);
        ...
        switch (deviceIndex) {
            ...
            case 1:
                return {
                    ...
                    deviceClasses: "w-20 h-44 md:w-28 md:h-60 lg:w-[35%] lg:h-[85%] overflow-visible",
                    screenClasses: "w-full h-full",
                } as const;
            case 2:
                return {
                    containerClasses: "items-end justify-end pr-8 pb-10",
                    deviceClasses: "w-20 h-44 md:w-28 md:h-60 lg:w-[37%] lg:h-[85%] overflow-visible",
                    screenClasses: "w-full h-full",
                } as const;
            case 3:
                return {
    ...
    const renderDeviceFrame = (deviceIndex: number, children: React.ReactNode) => {
        switch (deviceIndex) {
            // Laptop/Screen
            case 0:
                return (
    ...
    const carouselImages = (imagesFromApi && imagesFromApi.length > 0) ? imagesFromApi : [];
        ...
        <div className="w-full relative">
            <div className="relative">
                ...
                {carouselImages.length > 0 && (
                    <div className={`absolute inset-0 flex ${deviceConfig.containerClasses}`}>
                        <div className={deviceConfig.deviceClasses}>
                            {renderDeviceFrame(index, (
                                <Carousel
                                    autoplay
                                        ...
                                        {carouselImages.map((imageUrl, carouselIndex) => (
                                            <CarouselItem key={carouselIndex} className="h-full">
                                                <div className="relative w-full h-full">
                                                    <Image
                                                        src={imageUrl}
                                                        alt={`Device ${index} screen ${carouselIndex + 1}`}
                                                        fill
...
export const ProductImageWithOverlay = React.memo(ProductImageWithOverlayComponent);
