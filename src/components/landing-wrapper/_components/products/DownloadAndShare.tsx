...
type Props = {
  ...
  downloadText?: string; // hash like #product-mobile
};

export const DownloadAndShare: React.FC<Props> = ({
  downloadPlatforms = [],
  slug,
  downloadText,
}) => {
  ...
  return (
    <div className="flex flex-row items-start sm:items-center gap-3 sm:gap-4 pt-2 flex-wrap">
      ...
      <div className="flex items-center gap-3 sm:gap-4 sm:ml-auto relative">
        <p className="text-dark-100 hover:text-dark-200 text-sm font-semibold text-nowrap">
          {downloadText || "Download It Free"}
        </p>
        <button
          aria-label="Share"
          onClick={handleShare}
