...
export default function AboutSection({ data }: IProps) {
  ...
  const sectionTitle = aboutUsContent?.title || "LIVE MOSQUE - THE SERVICE IS COMPLETELY FREE TO ANY MASJID.";
  const sectionSubtitle = aboutUsContent?.subtitle || "Live Mosque helps display Iqama times in the Masjid and allows users to view them on their phones. It also offers events, notifications, free live streaming of sermons (khutbah) and azan. Choose between manual or automatic Iqama updates, and manage everything easily with the dedicated Imam (admin) app.";
  const featureBody1 = aboutUsContent?.body1 || "Masjid Clock is compatible with Wi-Fi and Offline Operation. A Wi-Fi connection is strongly recommended for real-time.";
  const featureBody2 = aboutUsContent?.body2 || "In offline mode , a mobile hotspot connection is required whenever you update the salah times.";
  const sinceYear = aboutUsContent?.main || aboutUsContent?.["main "] || "2019";
  const communityText = aboutUsContent?.main_subtitle || aboutUsContent?.["main_subtitle "] || "Years of contributing we invite you to become a part of the Live Mosque community.";
  const mediaUrl = aboutUsContent?.media_url || "/aboutUsRight.png";
  const buttonText = aboutUsContent?.readmore || aboutUsContent?.["readmore "] || "Read More";
  ...
  const computedContentPaddingY = vw >= BREAKPOINTS.desktop ? "8rem" : undefined;
  return (
    <section id="about-us" className="scroll-section relative w-full bg-primary-color pt-5 overflow-hidden">
      ...
      <div
        ...
        style={{ paddingTop: computedContentPaddingY, paddingBottom: computedContentPaddingY }}
      >
        {/* Left Side */}
        <div className="col-span-2 flex flex-col items-center lg:items-start text-center lg:text-left gap-5">
          ...
          <div className="flex flex-col items-center lg:items-start gap-2">
            ...
            <span className="text-white text-lg 2xl:text-xl font-medium mt-1">
              {sectionName}
            </span>
          </div>

          {/* Heading */}
          <h1 className="heading-responsive 2xl:max-w-2xl  font-bold text-secondary-color leading-snug font-cinzel-decorative max-w-xl">
            {sectionTitle}
          </h1>

          {/* Description */}
          <p className="text-white body-description max-w-xl 2xl:max-w-2xl pb-8 sm:pb-8">
            {sectionSubtitle}
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-10 md:gap-4 w-full max-w-xl">
            <div>
              <Card
                ...
                description={featureBody1}
              />
            </div>
            <div>
              <Card
                showInnerImage={false}
                description={featureBody2}
              />
            </div>
          </div>
          ...
          <div className="flex z-9 justify-center lg:justify-start w-full">
            <PrimaryButton className="bg-secondary-color mt-6 hover:bg-secondary-color/90 text-dark-100 !text-sm font-medium !px-7 !py-5.5" onClick={() => router.push('/about-us')}>
              {buttonText}
            </PrimaryButton>
          </div>
        </div>
        ...
        <div className="xl:hidden relative w-full pt-28 md:pt-32 max-w-sm sm:max-w-md mx-auto mt-8">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-6 w-[300px] max-w-[80vw] z-20">
            <div className="rounded-4xl bg-gradient-to-b from-white to-white/0 p-0.5 flex flex-col items-center justify-center text-center">
              <div className="rounded-4xl pb-6 pt-4 px-6 bg-primary-color flex flex-col items-center justify-center text-center">
                <h2 className="tracking-wide text-3xl sm:text-4xl font-bold text-secondary-color font-montagu">
                  Since &nbsp;
          ...
          <Image
            ...
            className="relative z-9999 w-full h-auto rounded-2xl lg:rounded-none object-contain"
            width={500}
            height={500}
          />
        </div>
          ...
          <div className="hidden md:block relative w-full">
            ...
            <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 bottom-full translate-y-8 sm:translate-y-10 md:translate-y-8 w-[370px] max-w-[80vw] z-10">
              <div className="mt-2 rounded-4xl bg-gradient-to-b from-white to-white/0 p-0.5 flex flex-col items-center justify-center text-center">
                <div className="rounded-4xl pb-10 pt-3 bg-primary-color flex flex-col items-center justify-center text-center">
                  <h2 className="tracking-wide text-[40px] font-bold text-secondary-color font-montagu">
                    Since &nbsp;
                    {/* <span className="inline-block bg-clip-text text-transparent bg-gradient-to-b from-primary-color to-secondary-color/30">
                      e
                    </span>{" "} */}
                  ...
                  <p className="mt-1 text-white/95 text-base leading-relaxed max-w-md">
                    {communityText}
                  </p>
                </div>
              </div>
